# Detection EVCS
# 1. Hacer para cada ataque
    # analisis de inestabilidad 
    # analisis de detección de los 3 mejores métodos
    # Descargar y cargar modelos
    # sin coordinador
# adaptar para aias dt
...
window_size = 1000
evcsADS_catboost = CatboostADS(window_size, x_init, y_init, "evcsADS_catboost")
evcsADS_lgbm = LGBMachineADS(window_size, x_init, y_init, "evcsADS_lgbm")
evcsADS_MLP = MultiLayerPerceptronADS(window_size, x_init, y_init, "evcsADS_MLP")
evcsADS_RF = RandomForestADS(window_size, x_init, y_init, "evcsADS_RF")
evcsADS_xgb = XGBoostADS(window_size, x_init, y_init, "evcsADS_xgb")
evcsADS_dnn = DeepANN_ADS(window_size, x_init, y_init, f"evcsADS_dnn", 8)
adsModels = [evcsADS_lgbm, evcsADS_MLP, evcsADS_MLP, evcsADS_RF, evcsADS_xgb, evcsADS_dnn]
...
def cargar_muestras_adversarias(eps, x_inc, y_inc, num_splits, adv_attack):
    adv_path = f'../../data/aexamples/evcs/{adv_attack}/test/adversarialexamples_eps{str(eps)}.csv'
...
def models_predict(x_sample, y_labels, y_det, det_model, models, threshold, adv_x_sample = None, which_model: list = None):
    ...
    for idx, model in enumerate(models):
        ...
        if adv_x_sample is not None:
            if idx in which_model:
                ...
                try:
                    ...
                    df = pd.read_csv("emissions_evcs.csv")
                    last = df.iloc[-1]
        ...
        if not adv_obj:
            ...
            start_time = time.time()
            tracker = EmissionsTracker(output_file="emissions_evcs.csv",log_level="error")
            tracker.start()
            cpu_thread = threading.Thread(target=monitor_cpu, args=(cpu_usage_samples, stop_flag))
            cpu_thread.start()
...
keras_pgdtim_model = tf.keras.models.load_model('../../data/models/keras_pgdtim_evcs.keras')
    ...
    for idx, detection_method in enumerate(detection_methods):
        ...
        for idx_eps, eps in enumerate(epsilon):
            ...
            for att_idx, attack in enumerate(attacks):
                ...
                #if idx == 0:
                #    print("Plot Attacks")
                #    plot(f"F1-score {attack} eps: {eps}", models_f1, f"./Experiments_results/evcs/attack/{adv_attack}/Attack[{att_idx}]_{attack}_eps_0.{str(eps)}_f1.pdf", "F1 Score")
                #    plot(f"Recall {attack} eps: {eps}", models_recall, f"./Experiments_results/evcs/attack/{adv_attack}/Attack[{att_idx}]_{attack}_eps_0.{str(eps)}_recall.pdf", "Recall")
                #    plot(f"Accuracy {attack} eps: {eps}", models_accuracy, f"./Experiments_results/evcs/attack/{adv_attack}/Attack[{att_idx}]_{attack}_eps_0.{str(eps)}_acc.pdf", "Accuracy")
                #    plot(f"EIR {attack} eps: {eps}", models_eir, f"./Experiments_results/evcs/attack/{adv_attack}/Attack[{att_idx}]_{attack}_eps_0.{str(eps)}_eir.pdf", "EIR")
#
                #print("Plot Detection")
                #plot_detection(f"Performance {attack} detection eps: {eps}", models_det_performance, f"./Experiments_results/evcs/detection/{detection_method}/{adv_attack}/Detection_[{att_idx}]_{attack}_eps_0.{str(eps)}_perf.pdf")
        ...
        plot_measurement(f"CPU %  usage for AE detection", models_cpu, f"./Experiments_results/evcs/detection/{detection_method}/{adv_attack}/CPU_Detection_[{att_idx}]_{attack}_eps_0.{str(eps)}_CPU.pdf", "%", epsilon)
        plot_measurement(f"Electrical consumption for AE detection", models_emmisions, f"./Experiments_results/evcs/detection/{detection_method}/{adv_attack}/KWH_Detection_[{att_idx}]_{attack}_eps_0.{str(eps)}_KWh.pdf", "KWh", epsilon)
        plot_measurement(f"Elapsed time for AE detection", models_emmisions, f"./Experiments_results/evcs/detection/{detection_method}/{adv_attack}/Time_Detection_[{att_idx}]_{attack}_eps_0.{str(eps)}_seconds.pdf", "Seconds", epsilon)  

print("EVCS finished successfully!!")